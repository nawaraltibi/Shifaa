...
class DoctorScheduleCubit extends Cubit<DoctorScheduleState> {
  ...
  Future<void> selectDateAndFetchSchedule(
    ...
    result.fold(
        ...
        final dayName =
        DateFormat('EEEE').format(newSelectedDate).toLowerCase();
        updatedSchedules
            .removeWhere((s) => s.dayOfWeek.toLowerCase() == dayName);
        updatedSchedules.addAll(daySchedule);
        emit(DoctorScheduleSuccess(
            updatedSchedules, _currentMonth, _selectedDate));
      },
    );
  }

  Future<void> fetchDoctorScheduleForDate(
      String doctorId, String dateString) async {
