...
class AppointmentRepositoryImpl implements AppointmentRepository {
  ...
  @override
  Future<Either<Failure, List<AppointmentEntity>>> getUpcomingAppointments({bool forceRefresh = false}) async {
    return _getAppointments(
      ...
      cacheLocal: (appointments) => localDataSource.cacheUpcomingAppointments(appointments),
      forceRefresh: forceRefresh,
    );
  }

  @override
  Future<Either<Failure, List<AppointmentEntity>>> getPreviousAppointments({bool forceRefresh = false}) async {
    return _getAppointments(
      getLocal: () => localDataSource.getLastPreviousAppointments(),
      getRemote: () => remoteDataSource.getAppointments('past'),
      cacheLocal: (appointments) => localDataSource.cachePreviousAppointments(appointments),
      forceRefresh: forceRefresh,
    );
  }
